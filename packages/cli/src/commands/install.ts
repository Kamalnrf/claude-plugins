...
import { detectRepoType, isValidClaudePlugin } from "../utils/validation";
	...
	try {
		...
		if (repoType === "marketplace") {
			await installMarketplace(
				{ identifierName: name, tempLocation, gitPath: `${url}.git` },
				s,
			);
		} else {
		...
		note(
			`Start a new Claude Code session to use the new plugin "${name}".`,
...
async function installMarketplace(
	{
		...
		tempLocation,
		gitPath,
	}: { identifierName: string; tempLocation: string; gitPath: `${string}.git` },
	...
	s.stop(
		`Plugin "${identifierName}" enabled from marketplace ${pc.dim(marketplaceName)}`,
	);
}
